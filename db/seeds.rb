...
10.times do
   proveedor =  Supplier.create name:Faker::Commerce.brand, email:Faker::Internet.email, phone:Faker::PhoneNumber.phone_number,arrive_date:Faker::Date.forward(days: 2),rfc:Faker::Code.nric
    Product.create bar_code:Faker::Commerce.product_name,category:Faker::Commerce.department,batch:Faker::Alphanumeric.alpha(number: 10),exp_date:Faker::Date.forward(days: 23),buying_price:Faker::Commerce.price, selling_price: Faker::Commerce.price,supplier_id:proveedor.id, units:Faker::Number.number(digits: 2)
